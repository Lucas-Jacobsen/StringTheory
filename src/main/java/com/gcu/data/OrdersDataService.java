...
public class OrdersDataService implements DataAccessInterface<ProductModel> {
	...
	public List<ProductModel> finalAll() {
		...
		try {
			...
			while (srs.next()) {
				products.add(new ProductModel(srs.getInt("PRODUCT_ID"), srs.getString("PRODUCT_CATEGORY"),
	...
	public ProductModel findById(int id) {
		...
		while (srs.next()) {
			...
			cat = srs.getString("PRODUCT_CATEGORY");
			System.out.println(srs.getString("PRODUCT_CATEGORY"));
			name = srs.getString("PRODUCT_NAME");
			System.out.println(srs.getString("PRODUCT_NAME"));
			des = srs.getString("PRODUCT_DESCRIPTION");
			System.out.println(srs.getString("PRODUCT_DESCRIPTION"));
			price = srs.getFloat("PRODUCT_PRICE");
			System.out.println(srs.getFloat("PRODUCT_PRICE"));
			...
			// System.out.println(pm.getProductCategory().toString()+
			// pm.getProductDescription().toString()+pm.getProductName().toString());
	...
	public boolean create(ProductModel product) {
		...
		// execute sql
		int rows = jdbcTemplateObject.update(sql, 0, product.getProductCategory(), product.getProductName(),
	...
	public boolean update(ProductModel t) {
		...
		// productUpdate = 1;
		String sql = "UPDATE PRODUCTS SET PRODUCT_NAME =?, PRODUCT_CATEGORY = ?, PRODUCT_DESCRIPTION = ?, PRODUCT_PRICE = ? WHERE PRODUCT_ID = '"
				+ productUpdate + "'";
		...
		int rows = jdbcTemplateObject.update(sql, t.getProductName(), t.getProductCategory(), t.getProductDescription(),
				t.getProductPrice());
		return rows == 1 ? true : false;
	}
